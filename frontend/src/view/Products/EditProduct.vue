  ...
  <div>
    <div class="max-w-3xl mx-auto p-6">
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <!-- Form -->
        <form @submit.prevent="editProduct" class="px-8 py-6 space-y-6">
          <!-- SKU + Name -->
          ...
          <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            <button
              @click="router.push({name:'SingleProduct',params:{id:route.params.id}})"
              type="button"
              class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium  cursor-pointer bg-gray-300 rounded-lg hover:bg-gray-600 transition focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >Back</button>
            <button
              type="submit"
              :disabled="loading"
...
onMounted(async () => {
  ...
  sku.value = productStore.product.sku
  name.value = productStore.product.name
  description.value = productStore.product.description
  type.value = productStore.product.type
  category_id.value = productStore.product.category_id
  unit_of_measure.value = productStore.product.unit_of_measure
  base_price.value = productStore.product.base_price
  is_active.value = productStore.product.is_active
  currency.value = productStore.product.currency
  vat_rate.value = productStore.product.vat_rate
})
...
const editProduct = async () => {
  ...
  try {
    await productStore.editProduct(productData, route.params.id)
    toast.success(`Product: ${productData.name} modified successfully!`)
    router.push({ name: 'SingleProduct', params: { id: route.params.id } })
  } catch (err) {
    console.error(err)
  }
}
