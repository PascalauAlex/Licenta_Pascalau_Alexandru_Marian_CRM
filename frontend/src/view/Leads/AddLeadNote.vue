<template>
  <div class="max-w-md mx-auto my-10">
    <h1 class="text-2xl font-bold mb-8 text-gray-800">Add Lead Note</h1>
    ...
    <form @submit.prevent="onSubmit" class="mx-auto">
      ...
      <div class="relative z-0 w-full mb-5 group">
        <label for="body" class="text-gray-500 mb-2 block text-sm">Description</label>
        <textarea
          v-model="body"
          id="body"
          rows="4"
          class="bg-neutral-secondary-medium border border-default-medium text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3.5 shadow-sm"
...
const onSubmit = async () => {
  ...
  try {
    await leadsStore.postLeadNote(formData)

    router.push({ name: 'SingleLead', params: { id: route.params.id } })
  } catch (err) {
    console.error('Saving the lead note failed', err)
  }
}
</script>
